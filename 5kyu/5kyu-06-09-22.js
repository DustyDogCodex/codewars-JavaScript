...
function chooseBestSum(t, k, ls) {
    let largestCount = 0;
    let recursiveTowns = function(townsTravelled, lastIndex) {
      townsTravelled = townsTravelled || [];

      if (townsTravelled.length === k) {
        let distanceTravelled = townsTravelled.reduce((a,b)=>a+b);
        if (distanceTravelled <= t && distanceTravelled > largestCount) {
          largestCount = distanceTravelled;
        }      
        return;
      }

      for (let i = lastIndex + 1 || 0; i < ls.length; i++) {
        recursiveTowns(townsSoFar.concat(ls[i]), i);
      }
    }
    recursiveTowns();
    
    return largestCount || null;
  }
